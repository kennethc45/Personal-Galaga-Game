 ...
 */
class Enemy(pic:Image, initPos:Vec2, private val bulletPic:Image, width:Int, height:Int) extends Sprite(pic, initPos, width, height) {
   ...
   */
  def shoot():Bullet = { 
    new Bullet (bulletPic, initPos.clone(), new Vec2 (0, 0.5), 70, 100)
  }
  ...
  def swarmMovement() = {         //swarmMovement Idea
    if (dirChange) {
      pos.x += 3 
      if (pos.x > 800) {
        dirChange = false
  ...
  override def clone():Enemy = {
    new Enemy(pic, pos.clone(), bulletPic, width, height)
  }
